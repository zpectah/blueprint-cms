...
import { FormResponseState, ITranslationsDetailForm } from '../../../types';
import { formResponseStateKeys } from '../../../enums';
import { Form, FormBlock, FormField, Input } from '../../../components';
import { TRANSLATIONS_DETAIL_FORM_ID } from '../constants';
import { useTranslationsContext } from '../context';
import { useTranslationsDetailForm } from '../hooks';

interface TranslationsDetailFormProps {
  onSubmit?: (state: FormResponseState, message: string) => void;
}

const TranslationsDetailForm = ({ onSubmit }: TranslationsDetailFormProps) => {
  const { detailData, responseSnack, setResponseSnack, setFormDirty } = useTranslationsContext();
  const { control, handleSubmit, formState } = useTranslationsDetailForm(detailData);

  const submitHandler: SubmitHandler<ITranslationsDetailForm> = (data, event) => {
    // TODO #submit-handler
  ...
  return (
    <>
      <Form id={TRANSLATIONS_DETAIL_FORM_ID} onSubmit={handleSubmit(submitHandler)}>
        <FormBlock>
