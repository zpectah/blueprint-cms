...
const MessagesDetailForm = ({ onSubmit }: MessagesDetailFormProps) => {
  const { detailData, responseSnack, setResponseSnack, setFormDirty } = useMessagesContext();
  const { control, handleSubmit, formState } = useMessagesDetailForm(detailData);

  const submitHandler: SubmitHandler<IMessagesDetailForm> = (data, event) => {
    // TODO #submit-handler
  ...
  return (
    <>
      <Form id={MESSAGES_DETAIL_FORM_ID} onSubmit={handleSubmit(submitHandler)}>
        <FormBlock>
